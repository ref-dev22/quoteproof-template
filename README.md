...
| Piece | What QuoteProof uses it for | Code |
| --- | --- | --- |
| Chainlink HBAR/USD proxy on Hedera Testnet | Supplies the price and exact round for a quote. | [Registry oracle read](packages/hardhat/contracts/QuoteProofRegistry.sol) |
| Hedera Smart Contract Service | Stores each issuer/nonce fingerprint and emits `QuoteRecorded`. | [Registry contract](packages/hardhat/contracts/QuoteProofRegistry.sol) |
| Hedera Consensus Service | Anchors a confirmed receipt in a submit-key topic. | [Topic creation](packages/nextjs/scripts/createHcsTopic.cjs), [anchor route](packages/nextjs/app/api/quote/hcs/anchor/route.ts) |
| Mirror Node REST | Reads an HCS message and checks its configured payer. | [HCS verify route](packages/nextjs/app/api/quote/hcs/verify/route.ts) |
| JSON-RPC relay | Reads the contract, stored commitment and historical oracle round. | [Preview route](packages/nextjs/app/api/quote/preview/route.ts), [compare route](packages/nextjs/app/api/quote/compare/route.ts) |

## Docs map

- **Tutorial:** [Your first verifiable receipt in 15 minutes](docs/TUTORIAL.md).
- **How-to:** [Change, record and troubleshoot](docs/HOW-TO.md).
- **Reference:** [Routes, receipt, commands and environment](docs/REFERENCE.md).
- **Explanation:** [Design, trust and limits](docs/EXPLANATION.md).

[Security audit](docs/security.md). [MIT license and upstream notice](LICENSE).

AI-assisted implementation and review were used. Developers can reproduce the checks and should inspect the code and dependency advisories before adapting this template.
