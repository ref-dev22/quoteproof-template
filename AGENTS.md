...
| Export and CLI verify | Receipt export / `npm run verify:quote` | `packages/nextjs/components/quoteproof/ReceiptProofCard.tsx`, `packages/hardhat/scripts/verifyQuote.ts`, `packages/hardhat/utils/quoteReceipt.ts` | Run the verifier command in `docs/REFERENCE.md`; independent expected values matter. RPC comparison is optional. |
