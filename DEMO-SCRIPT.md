...
4. **40–52s — Show the offline boundary.** Click `Export receipt JSON`, then run `npm run verify:quote -w @sh/hardhat -- --input <downloaded-file>`. The valid result is calculation-checked and explicitly `onChainVerified: false`; the separate UI checks are what compare the historical oracle observation and stored commitment.
