...
4. **40–52s — Show the offline boundary.** Click `Export receipt JSON`, then stop relying on the web app and run `npm run verify:quote -w @sh/hardhat -- --input <downloaded-file> --compare-stored --rpc-url https://testnet.hashio.io/api`. The valid result is calculation-checked and explicitly `onChainVerified: false`; the CLI's separate `historicalOracle` and `recordedComparison` results perform the read-only checks directly. If an independently selected expected quote is available, add its commitment, issuer and nonce flags; otherwise `expectedQuote` remains `not_supplied`.
