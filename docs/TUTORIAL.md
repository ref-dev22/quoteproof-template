...
The repo includes the [published receipt JSON](../examples/receipt-testnet.json). From `quoteproof`, run the [historical verifier command](HOW-TO.md#inspect-the-historical-receipt):
...
For a different feed or receipt field, follow [Swap the price feed](HOW-TO.md#swap-the-price-feed) or [Add a receipt field](HOW-TO.md#add-a-receipt-field). A new receipt requires the owner-authorized steps in [Record a new receipt on Testnet](HOW-TO.md#record-a-new-receipt-on-testnet); this wallet-free tutorial does not run them.
